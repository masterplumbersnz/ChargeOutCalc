...
(function () {
    ...
    function getHead() { return document.querySelector('.mpc-table-head'); }

    function ensurePlaceholder(head) {
        if (!placeholder) {
            placeholder = document.createElement('div');
            placeholder.style.display = 'none';
            head.parentNode.insertBefore(placeholder, head);
        }
    }

    function pin(head, navHeight) {
        ensurePlaceholder(head);
        var rect = head.getBoundingClientRect();
        placeholder.style.height = rect.height + 'px';
        placeholder.style.display = 'block';
        head.style.position = 'fixed';
        head.style.top = navHeight + 'px';
    ...
    window.addEventListener('message', function (e) {
        if (e.origin !== 'https://masterplumbers.org.nz') return;
        if (!e.data || e.data.type !== 'mpc-scroll-sync') return;

        var head = getHead();
        if (!head || getComputedStyle(head).display === 'none') return; // hidden on mobile layout

        var navHeight = e.data.navHeight || 0;
        var referencePoint = pinned ? placeholder : head;
        var naturalTop = e.data.iframeTop + referencePoint.getBoundingClientRect().top - (pinned ? 0 : 0);

        if (naturalTop <= navHeight) {
            pin(head, navHeight);
        } else if (pinned) {
            unpin(head);
        }
